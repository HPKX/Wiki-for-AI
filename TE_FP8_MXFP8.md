...
### 1.1 核心收益：降低核心 GEMM 主路径成本

以 `Linear` 层为例，一次训练迭代的主要低精度优化对象通常是三次核心 GEMM：

```text
forward:   y  = x @ w^T
backward:  dx = dy @ w
backward:  dw = dy^T @ x      # 等价地，d(w^T) = x^T @ dy
```

若将输入展平为 `M x K`、权重视为 `N x K`，上述三次 GEMM 的计算量均约为 `2MNK` FLOPs，因此它们是 `Linear` 训练中同阶的主计算路径。公开资料中的 FP8/MXFP8 端到端收益也主要受 GEMM 占比、低精 GEMM 吞吐提升，以及量化/通信额外开销共同决定。

> FP8/TE 的核心不是“把模型压成 8 bit 存储”，而是“把 forward、dgrad、wgrad 这三条核心 GEMM 主路径稳定地纳入低精度训练”。
...
从 `FP8 training`、`run in FP8` 这类简化说法出发，容易把 TE 理解成类似 `model.to(FP8)` 的全局 dtype 切换。更准确地说，TE 是在模块边界内把高精度 tensor 转成低精度 kernel 可消费的表示，并维护这种表示在前向、反向、通信和状态更新中的生命周期。
...
这说明 TE 的 FP8 路径可以理解为一条完整的“量化 -> GEMM -> 状态更新”闭环。
...
在 FP8 训练路径中，低精度 GEMM kernel 消费的是一组带缩放信息的量化表示，主要包括：

- 量化后的数据
- 对应的 scale / scale_inv
- 知道如何解释它们的 GEMM kernel

从数值表示的角度看，理解重点应放在“量化数据 + scaling tensor + scaled matmul”这一整套表示与执行方式上，而不是只看 `torch.float8_e4m3fn` / `torch.float8_e5m2` 这样的 dtype 名称。
...
核心特征可以按“scale 单位 -> 矩阵方向 -> 表示后果”理解：

- 基本单位是 OCP MX block：连续 `32` 个元素共享一个缩放因子；block size 固定为 `32`，不是 TE 在 recipe 层暴露的可配置项
- 缩放因子采用 `E8M0`，即 power-of-two microscale；它和 FP8 数据共同构成 MXFP8 表示
- 对二维矩阵，block 仍然是一维的：需要沿某个方向把矩阵划分为长度 `32` 的连续 block，而不是让一个二维 tile 共享同一个 scale
- rowwise 与 columnwise 对应两种不同的 block 划分方向，也对应后续 GEMM 对输入布局的不同消费方式
- 因为 scale 绑定在特定方向的一维 block 上，`quantize(x).T` 与 `quantize(x.T)` 不等价；TE 在 MXFP8 路径中通常需要分别维护 rowwise 和 columnwise 的量化副本

这里的 block scaling 指 `MXFP8` 的一维 microscaling；二维 tile 级 blockwise scaling 是另一类量化策略，不是本节讨论的 `MXFP8BlockScaling` 语义。
...
对训练脚本而言，最常见的用户侧入口是 `te.autocast(...)`。它定义一次低精度执行上下文：启用当前 recipe、检查硬件与 recipe 支持、让 TE 模块在 forward 期间拿到统一的 quantizer / fp8 meta，并在退出上下文时触发 delayed scaling 的 forward 侧状态收尾。
...
autocast 只做“状态切换”
  ...
  -> general_gemm() 汇总 A/B 量化张量与 layout
     可选传入 quantization_params=output_quantizer，用于让 GEMM 输出继续量化
     可选传入 ub/ub_type/extra_output/bulk_overlap，用于通信与 GEMM overlap
...
`usage` 的含义是：在量化前声明这份量化结果后续会被哪些 GEMM 方向消费。它不是单纯的布局标签，而会影响量化张量中实际生成和保留哪些数据视图。

这组设置分别对应三个张量后续会被怎样使用：

- `weight`：forward 数学上是 `y = x @ w^T`，但实现中把 `w` 以原始布局传入 `general_gemm(w, x, layout="TN")`，转置语义由 GEMM layout 表达；因此 forward 阶段只声明原始布局对应的 rowwise 视图
- `input`：forward 需要 rowwise；若需要计算 `dw = dy^T @ x`，backward 还会复用 input 的 columnwise 视图，因此 `columnwise` 跟随 `weight_requires_grad`
- `grad_output`：它服务 backward；dgrad 需要 rowwise，wgrad 需要 columnwise，因此也按 `weight_requires_grad` 决定是否提前声明 columnwise

影响是：打开 `columnwise` 能让 backward 复用已有量化缓存，但可能带来额外转置和缓存开销。对普通 `FP8`，如果设备不支持 `non-TN FP8 GEMM`，columnwise 通常对应一份转置后的 FP8 buffer；若后续才发现需要某个视图，则通过 `update_usage(...)` 再补齐。
...
│  │     数学上: dx = dy @ w                                             │   │
│  │     源码中: general_gemm(w, dy_fp8, layout="NN")                    │   │
│  │     ├─ w 使用 forward 后补齐 / 缓存的 columnwise FP8 数据           │   │
│  │     └─ 输出为高精度 (BF16/FP16)                                     │   │
│  │                                                                     │   │
│  │  3. wgrad GEMM: 计算 grad_weight                                    │   │
│  │     数学上: dw = dy^T @ x                                           │   │
│  │     源码中: general_gemm(x, dy_fp8, layout="NT")                    │   │
...
对 `MXFP8`，backward 数据消费要从两条 GEMM 的输入视图理解。这里的 `usage` 不是给 tensor 贴一个 row / column 标签，而是声明量化结果后续会以哪种 GEMM 视图被 kernel 消费，并据此决定是否需要保留对应的 `data + scale_inv`。

先看 `Linear` 的数学关系。若 `x` 为 `[M, K]`，`w` 为 `[N, K]`，forward 为 `y = x @ w^T`，且 `dy` 为 `[M, N]`，则 backward 的两个主要梯度为：
...
其中 `dw` 的形状仍是 `[N, K]`，与 PyTorch/TE 中的 `weight` 形状一致；若从 `w^T` 这个 forward operand 角度看，等价地有 `d(w^T) = x^T @ dy`。

`BasicLinear` backward 中，这两条公式对应的源码调用分别是：
...
`layout` 的两个字符分别描述 A / B operand 在 GEMM 中是否转置；对量化 tensor 来说，它也决定 kernel 需要哪一种量化视图。对应关系可以整理为：

| backward GEMM | 数学公式 | TE 调用 | operand 消费方式 | 需要的 MXFP8 表示 |
| --- | --- | --- | --- | --- |
| dgrad | `dx = dy @ w` | `general_gemm(w, dy, layout="NN")` | `W` 作为 A operand 的非转置视图，`dY` 作为 B operand 的非转置视图 | `W columnwise` + `dY rowwise` |
| wgrad | `dw = dy^T @ x` | `general_gemm(x, dy, layout="NT")` | `X` 作为 A operand 的非转置视图，`dY` 作为 B operand 的转置视图 | `X columnwise` + `dY columnwise` |

关键点在于，`MXFP8` 的 rowwise / columnwise 不是同一份 FP8 buffer 的两个逻辑解释。每个 scale 绑定在长度为 `32` 的连续 block 上，rowwise 和 columnwise 对应不同的 block 连续方向。GEMM kernel 沿 reduction 维读取 FP8 数据时，也需要读取同方向的 `scale_inv`；方向不匹配时，数据和 scale 的配对关系就错了，不能靠调整 stride、transpose flag 或 layout 参数修正。

这也解释了 `grad_output_quantizer` 的设置：
...
如果需要 dgrad，`dY` 要准备 rowwise；如果需要 wgrad，`dY` 要准备 columnwise。两条 backward GEMM 同时存在时，TE 可以通过 usage 在量化或 `all-gather` 路径上同时准备两种表示。反过来说，若当前只有 `dY rowwise`，它只能服务 dgrad，不能直接作为 wgrad 的 `dY columnwise` 输入；需要重新量化，或单独准备 columnwise 表示。
...
下面分成 forward 和 backward 两张表；每张表内部仍按通信路径归并，避免把同一路径按 usage 拆成多行。

forward 路径：

| 并行策略 / 场景 | 引入的通信 | 通信对象 | 是否属于 MXFP8 低精通信 | 说明 |
| --- | --- | --- | --- | --- |
| `column parallel + sequence parallel` | GEMM 前 `all-gather` | `input rowwise data + scale_inv` | 是 | column TP 按输出特征维切权重，sequence parallel 又按第一维切 activation；forward GEMM 需要完整 `input`，因此可在 gather 路径上直接产出 rowwise `MXFP8Tensor` |
| `row parallel`（SP / 非 SP） | GEMM 后归约 | SP: `reduce_scatter_along_first_dim(y)`；非 SP: `all_reduce(y)` | 否 | row TP 按输入特征维切分，forward 本地 GEMM 只得到 `y` 的 partial sum。SP 下归约后还要把第一维切回各 rank；非 SP 下各 rank 保留完整 `y`。源码中 row TP 会关闭 quantized output，所以合并的是高精度 `y` |
| `context parallel` attention | attention 内部 `p2p` / `all-gather` / `a2a` / `a2a+p2p` | `KV/QKV` 或 attention 中间量 | 否 | 这里可能存在 FP8 attention 的 `uint8` 通信，但 `DotProductAttention` 的 FP8 attention 主线是 delayed/current per-tensor scaling，不是 `MXFP8Tensor` 的 `data + scale_inv` 通信 |
| `DTensor / FSDP2` 分片参数 | forward 参数 `all-gather` | 分片权重的 rowwise `data + scale_inv` | 是 | `reshard_after_forward=True` 时 forward 只 gather rowwise；若 forward 后参数不立即重分片，则 forward 可能同时 gather rowwise 和 columnwise 供 backward 复用 |

backward 路径：

| 并行策略 / 场景 | 发生子阶段 | 引入的通信 | 通信对象 | 是否属于 MXFP8 低精通信 | 说明 |
| --- | --- | --- | --- | --- | --- |
| `row parallel + sequence parallel` | dgrad / wgrad 前 | GEMM 前 `all-gather` | dgrad 消费 `grad_output rowwise`；wgrad 消费 `grad_output columnwise` | 是 | backward 入口会先 gather `grad_output`。若 dgrad 和 wgrad 都存在，usage 可以在同一路径中准备两种表示；若只有 rowwise 结果，则不能直接复用为 wgrad 的 columnwise 输入 |
| `column parallel + sequence parallel` | wgrad 前 | GEMM 前 `all-gather` | `input columnwise data + scale_inv` | 是 | forward 保存的是本地 `x_local`，不是 gather 后的完整 `x`；wgrad 需要完整 `input` 的 columnwise 表示，因此仍可能在 backward 中重新 gather |
| `column parallel`（SP / 非 SP） | dgrad 后 | GEMM 后归约 | SP: `reduce_scatter_along_first_dim(dx)`；非 SP: `all_reduce(dx)` | 否 | column TP 按输出特征维切分，dgrad 本地 GEMM 只得到 `dx` 的 partial sum。SP 下归约后把第一维切回各 rank；非 SP 下各 rank 保留完整 `dx`。源码中 column TP 会关闭 quantized grad input，所以合并的是高精度 `dx` |
| `context parallel` attention | attention backward | `p2p` / `all-gather` / `a2a` / `a2a+p2p` | attention backward 中间量 | 否 | 与 forward 类似，这里可能有 FP8 attention 通信，但不是 `MXFP8Tensor` 的块级 `data + scale_inv` 通信 |
| `DTensor / FSDP2` 分片参数 | backward 参数 gather | 参数 `all-gather` | 分片权重的 columnwise `data + scale_inv` | 是 | `reshard_after_forward=True` 时 backward 只 gather columnwise；若 forward 已保留完整参数，则这里可能复用已有表示 |
| `data parallel / DDP` | 梯度同步 | 梯度 `all-reduce` | 参数梯度 | 否 | 这是优化器可见的参数梯度同步，不是 TE GEMM 输入侧的 MXFP8 表示通信；通常仍按训练栈配置的高精度 / 梯度 dtype 处理 |
